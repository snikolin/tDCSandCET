function trials = sn_CETtrials(data)
% Create EEG trials
EEG             = find(strcmp({data.event.type},'eeg'));
EEG_LABELS      = {'Open EEG','Closed EEG'};
...
nback{1}    = 1:(EEG(1) - 1); % Pre
nback{2}    = (EEG(2) + 1):length(data.event); % Post

row = length(EEG);
NBACK_LABELS = {'Practice NBACK','NBACK'};
